...
bool if3D = false;
bool if3DSurface = false;
bool if3DSolid = false;
...
bool CAirfoilDesigner::fModify = false;
...
void CAirfoilDesigner::read3DDataNPlot()
{
    QString fileName = QFileDialog::getOpenFileName(this, ("Open 3D Data"),
                                                     ".",
                                                     ("DataTXT (*.txt)"));
    ...
    QFile file(fileName);
     ...
     while (!file.atEnd()) {
         ...
         if(flag==0)
         {
             ...
             PtsSolidX = new float*[Nj]; PtsSolidY = new float*[Nj]; PtsSolidZ = new float*[Nj];

             for(int j=0; j<Nj; j++)
             {
                 PtsSolidX[j] = new float[Ni];
                 PtsSolidY[j] = new float[Ni];
                 PtsSolidZ[j] = new float[Ni];
                 for(int i=0; i<Ni; i++)
                 {
                     in >> PtsSolidX[i][j] >> PtsSolidY[i][j] >> PtsSolidZ[i][j];
                 }
             }
             ...
             in >> Nsurf;
             PtsSurfX  = new float[Nsurf]; PtsSurfY  = new float[Nsurf]; PtsSurfZ  = new float[Nsurf];
             for(int i=0; i<Nsurf; i++)
             {
                 in >> PtsSurfX[i] >> PtsSurfY[i] >> PtsSurfZ[i];
             }
...
void CAirfoilDesigner::mousePressEvent(QMouseEvent *e)
{
    ...
    {
        fModify = true;
        _anchor1 = old_pos.x()-5.0;
        _anchor2 = old_pos.x()+5.0;
        _anchor3 = old_pos.y() +5.0;
        _anchor4 = old_pos.y() -5.0;
    }
}
...
void CAirfoilDesigner::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if(fModify)
    {
        ...
        {
            //provide handle to the point...
            if((_anchor1 > _anchorXu[i] && _anchor2 < _anchorXu[i] && _anchor3 < _anchorYu[i] && _anchor4 > _anchorYu[i]))
            {
                //glVertex2f(_anchor1, _anchor3)
                //glVertex2f(_anchor2, _anchor3)
                //glVertex2f(_anchor2, _anchor4)
                //glVertex2f(_anchor1, _anchor4)
                break;
            }
...
////////////////////////////////////////////////////////////////////////
    ...
    if(if3DSurface)
    {
        for(int i=0; i<Nsurf-1; i++)
        {
            glBegin(GL_QUADS);
            glColor3f(0.0, 1.0, 0.0);
            glVertex3f(PtsSurfX[i], PtsSurfY[i], PtsSurfZ[i]);
            glVertex3f(PtsSurfX[i+1], PtsSurfY[i+1], PtsSurfZ[i+1]);
            glVertex3f(PtsSurfX[i+1+Nsurf], PtsSurfY[i+1+Nsurf], PtsSurfZ[i+1+Nsurf]);
            glVertex3f(PtsSurfX[i+Nsurf], PtsSurfY[i+Nsurf], PtsSurfZ[i+Nsurf]);
        }
    }
    ...
    if(if3DSolid)
    {
        for(int j=0; j<Nj-1; j++)
        {
            for(int i=0; i<Ni-1; i++)
            {
                glBegin(GL_QUADS);
                glColor3f(0.0, 1.0, 0.0);
                glVertex3f(PtsSolidX[i][j], PtsSolidY[i][j], PtsSolidZ[i][j]);
                glVertex3f(PtsSolidX[i+1][j], PtsSolidY[i+1][j], PtsSolidZ[i+1][j]);
                glVertex3f(PtsSolidX[i+1][j+1], PtsSolidY[i+1][j+1], PtsSolidZ[i+1][j+1]);
                glVertex3f(PtsSolidX[i][j+1], PtsSolidY[i][j+1], PtsSolidZ[i][j+1]);
            }
        }
